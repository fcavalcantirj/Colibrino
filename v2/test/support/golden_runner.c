...
typedef struct {
  ...
  uint32_t start_ms;
  uint32_t end_ms;
  int expect_impulses; /* -1 = unconstrained */
  int expect_clicks;   /* -1 = unconstrained */
  uint32_t impulses;
  uint32_t clicks;
} segment_t;
...
static const char *const kTsvHeader =
    "id\tkind\tstage\tstart_ms\tend_ms\texpect_impulses\texpect_clicks\t"
    "expect_source\tnote";
...
static int parse_expect(const char *tok) {
  if (strcmp(tok, "*") == 0) {
    return -1;
  }
...
/* Parses NAME.labels.tsv; returns segment count or -1. */
static int read_labels(const char *path, const char *shown, segment_t *segs) {
  ...
  while (fgets(line, sizeof line, f) != NULL) {
    ...
    if (count >= MAX_SEGMENTS) {
      printf("FAIL: %s: too many segments\n", shown);
      fclose(f);
      return -1;
    }
    char *fields[9] = {0};
    ...
    if (nf < 7) {
      printf("FAIL: %s: short labels row %d\n", shown, count + 1);
      fclose(f);
      return -1;
    }
    ...
    snprintf(s->stage, sizeof s->stage, "%s", fields[2]);
    s->start_ms = (uint32_t)strtoul(fields[3], NULL, 10);
    s->end_ms = (uint32_t)strtoul(fields[4], NULL, 10);
    s->expect_impulses = parse_expect(fields[5]);
    s->expect_clicks = parse_expect(fields[6]);
...
static bool run_fixture(const char *csv_path, const char *shown,
  ...
  while (fgets(line, sizeof line, f) != NULL) {
    ...
    for (int i = 0; i < nseg; ++i) {
      /* Inclusive window on the event's own device timestamp. */
      if (CV2_MS_DIFF(s.t_ms, segs[i].start_ms) < 0 ||
          CV2_MS_DIFF(segs[i].end_ms, s.t_ms) < 0) {
        continue;
      }
      if (impulse) {
        ++segs[i].impulses;
...
int golden_main(golden_mode_t mode, const char *traces_dir, int oracle_gate) {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    for (int k = 0; k < nseg; ++k) {
      ...
      const int expect =
          mode == GOLDEN_MODE_IMPULSES ? s->expect_impulses : s->expect_clicks;
      const uint32_t got = mode == GOLDEN_MODE_IMPULSES ? s->impulses : s->clicks;
      if (expect < 0) {
        continue;
      }
      ++checked;
      if ((uint32_t)expect != got) {
        printf("FAIL: %s segment %s (%s, %s): expected %d %s, got %u\n", shown,
               s->id, s->kind, s->stage, expect, what, got);
        ++failures;
      }
    }
