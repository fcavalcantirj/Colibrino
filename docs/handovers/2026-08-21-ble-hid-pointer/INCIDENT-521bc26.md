...
## PLAN-r2 Step C — BLE candidate boots clean over the cable lane (Builder, 2026-08-22) [REAL]

Felipe's go ("anything that works"); he long-pressed into download mode. Cable lane executed exactly as planned: otadata read first (entry0 seq 7 VALID = app0 `254fb7e`, entry1 seq 8 VALID = app1 active), `esptool write_flash 0x340000 firmware-upload-ff1f7d2.bin` (SHA-256 `e25eb5bb…`, "Hash of data verified", otadata untouched; record `cable-flash-ff1f7d2-20260822T015437Z.txt`), single-click reset by Felipe. Evidence (`serial-stepC-ff1f7d2-*.log`, TCP capture `capture-20260821T225620`):
- USB-Serial/JTAG console window (the 1 s `COLIBRINO_BOOT_CONSOLE_MS` hold): `rst:0x15 (USB_UART_CHIP_RESET),boot:0x2b (SPI_FAST_FLASH_BOOT)` → `EVENT,BOOT,reset=UNKNOWN,raw=21,slot=app1,ota_state=VALID,attempts=1,last_stage=2,last_action=NONE,build=ff1f7d2` → `EVENT,LAST_CRASH,…sha=2f533da347509066,stale=1…` → `EVENT,BOOT,DEADLINE,armed=1,ms=45000,rc=0` — printed before `USB.begin()` exactly as designed (raw 21 = USB_UART_CHIP_RESET, classified cold → attempt 1).
- Composite CDC (logger now asserting DTR+RTS): `EVENT,BOOT_REPLAY` + `EVENT,LAST_CRASH_REPLAY` on attach, then `EVENT,OTA_IMAGE,CONFIRMED,reason=HEALTHY,state_before=VALID,state=VALID,rc=0` at +10 s.
- Wi-Fi joined with the BT controller enabled (the 521bc26 failure point) — no abort; `WIFI_PS_MIN_MODEM` fix validated on hardware.
- TCP: greeting `EVENT,TELEMETRY,CONNECTED,ff1f7d2,dropped=0`; 50 STATUS in 50 s, 0 `DROPPED`; STATUS `armed=0,calibrated=1,ota=READY,tele=1,ble=IDLE,hid=USB,bonded=0,repair=0,heap=144724,heap_largest=131060,reset=UNKNOWN,img=VALID,lc=2`.
- Budget: internal heap 213.6 KB (noble, Wi-Fi up) → 144.7 KB (BLE host+controller up) ⇒ BLE bring-up ≈ 68 KB, inside PLAN-r1's predicted 45–80 KB band; free ≥ 80 KiB and largest ≥ 40 KiB stop thresholds respected.
- Incident requirements 1–4 are all closed: root cause `[REAL]`, cable+console lane used for the candidate, scope deviation explained, all through PLAN-r2. Next: PLAN-r1 Step 5 physical BLE acceptance (cable-free, Felipe's hands) and the 30 s `COEX_IMU` coexistence measurement against the ~170 Hz / 0-drop baseline.
