...
## 2026-08-22T02:15:22Z — HEAD f57cd72 (branch `agent/ble-hid-pointer`)

BLE HID pointer commissioned on the StickS3, after a root-caused boot-loop incident, with a permanent OTA safety net. Sequence: the Builder's `521bc26` (bounded bonded NimBLE HID transport behind a single `MouseOutputPolicy` choke point, `f29a72b`) was OTA-deployed and boot-looped; the Architect recovered by cable and filed `docs/handovers/2026-08-21-ble-hid-pointer/INCIDENT-521bc26.md`; PLAN-r2 (`d78d2e2`, APPROVED `03e2986`) was executed end to end on 2026-08-22. **Root cause `[REAL]`** (read-only `esptool read_flash` of the untouched `coredump` partition + `esp-coredump` decode against the retained `521bc26` ELF, then self-reported by the device over telemetry as `EVENT,LAST_CRASH_REPLAY`): task `wifi`, `abort()` in `pm_set_sleep_type` ← `wifi_set_ps_process` ← `ieee80211_ioctl_process` — `startOtaNetworking()` called `WiFi.setSleep(false)` (`WIFI_PS_NONE`) after `ble_mouse.begin()` had enabled the BT controller; libcoexist aborts with "Should enable WiFi modem sleep when both WiFi and Bluetooth are enabled"; the official ESP-IDF v4.4.7 ESP32-S3 Wi-Fi guide states "Disabling modem sleep entirely is not possible for Wi-Fi and Bluetooth coexist mode" (relaxed only in IDF 5.0+). Not NimBLE, not heap, not brownout, not a hang. Fix `a961cef`: the BLE build keeps `WIFI_PS_MIN_MODEM`. Why it looped forever: Arduino-ESP32 2.0.17's `initArduino()` marks a fresh OTA image VALID before `setup()` unless `verifyRollbackLater()` returns true, so the bootloader's rollback (which is compiled in and proven to run NEW→PENDING_VERIFY→VALID on this unit: otadata readback seq 7/8 VALID, Step B booted `PENDING_VERIFY` then self-confirmed) never had a chance; and `USB.begin()` hands the single USB PHY to TinyUSB via RTC-domain bits that survive software resets, so after the first boot no console existed on the USB-C port at all. **Safety net `[REAL]`** (`027442b`, permanent): `extern "C" bool verifyRollbackLater()` (C linkage is mandatory — a C++ definition mangles and silently no-ops) + health-gated `esp_ota_mark_app_valid_cancel_rollback()` 10 s after `setup()` (`EVENT,OTA_IMAGE,CONFIRMED,reason=HEALTHY,state_before=PENDING_VERIFY,state=VALID` observed live), 45 s boot-health deadline on a dedicated task, RTC-noinit attempt budget (3; only PANIC/WDT resets count — power-on, side button, brownout, `esp_restart`, USB-JTAG reset restart the budget), early static-init mark, OTA-start confirm guard (Arduino's `Update` never calls `esp_ota_begin`), portable `BootHealthPolicy` with 9 native tests + 3 Wokwi checks; boot breadcrumbs `EVENT,BOOT,reset=,raw=,slot=,ota_state=,attempts=,last_stage=,last_action=,build=` and `EVENT,LAST_CRASH,task=,pc=,cause=,vaddr=,sha=,stale=,bt=…` printed with `printf()` before `USB.begin()` (the one USB-Serial/JTAG window per hard reset — observed on the Step C cable boot) and replayed as `*_REPLAY` on every CDC/TCP attach; STATUS tails `reset=,img=,lc=`; post-link gate `scripts/check_image.py` (rollback hook disassembly must be constant-true, `esp_reset_reason` linked, breadcrumb strings present, true internal-DRAM = `.dram0.dummy+.data+.bss` ≤ 200 KB — PlatformIO's RAM figure hides the IRAM shadow: BLE's real static cost is ≈ 17.7 KB, not 5.7); `scripts/serial_console.py` (reconnecting JTAG+CDC logger; DTR+RTS asserted only on the Colibrino CDC). Also fixed: `-DCONFIG_BT_NIMBLE_SM_SC_ONLY=1` (would have refused every encrypted HID read under Just-Works) removed; NimBLE host task 8 KB. **Hardware evidence 2026-08-22 `[REAL]`**: Step B OTA of the BLE-less `ff1f7d2-noble` proved PENDING_VERIFY→CONFIRMED, breadcrumbs, and the on-device dump readout; Step C cable flash of `ff1f7d2` (BLE build, app1) booted clean with Wi-Fi + BLE (`ble=IDLE`, `hid=USB` with the data cable, heap 144.7 KB / largest 131 KB; BLE bring-up ≈ 68 KB vs the 45–80 KB prediction). BLE bench (cable-free, Felipe's hands): 60 s Just-Works window → `SECURE,bonded=1` with output still locked; 2 s hold arms (`ARMED,hid=BLE`); head motion moves the Mac cursor — operator verdict "horizontal very good, vertical not that much" (pointer-feel tuning item, not a transport defect); physical lock; Mac-Bluetooth-off while armed → `LOCKED,reason=TRANSPORT_TOPOLOGY` + `BLE,DISCONNECTED,reason=531` with no self-advertising; 30 s whitelist reconnect window → `SECURE,bonded=1`, still locked until a fresh 2 s arm ("perfect"); a mid-session re-encryption event kept the armed state. Coexistence gate (armed over BLE with the TCP mirror attached; three recorded 30 s `COEX_IMU` windows, `capture-20260821T230348`): 165.8 / 165.5 / 164.8 Hz, device-time gaps p50 5.00/4.99/4.98 ms, p95 7.1–7.9 ms, p99 31.0–31.1 ms, max 34.9–36.7 ms, 1571/1284/115 BLE reports, 0 failures, 0 rows missing, `DROPPED` 0 (cumulative since boot) vs the 2026-08-18 wireless baseline 170.3 Hz / p50 4.98 / p95 6.99 / p99 28.07 / max 30.94 ms — rate within 5 %, p99 +3 ms (< 5 ms), max +4–6 ms (same ~35 ms class as earlier TCP runs; noted, not failed). Session: ~25 min connected, 4945 BLE reports, 0 failures, heap 112–137 KB, battery 100 → 90 % over ≈ 15 min of BLE + Wi-Fi (modem sleep) + display + streaming (observation only). Sizes at `ff1f7d2`: RAM 104,788 B (32.0 %), Flash 1,255,389 B (37.6 %), internal-DRAM static 176,720 B; noble 100,360 / 1,129,517; no-secrets 46,528 / 820,593. Gates: native 33/33, tooling 29/29, Wokwi 18 checks, `CHECK_IMAGE,PASS`. **Not yet validated**: the USB-cable topology switch while BLE is secure (cable unavailable at the end of the session), a dedicated 10-minute armed soak, the unknown-peer rejection and 5 s forget+re-pair flows, Stage 2 (BLE pointer + Mac camera click), vertical feel tuning, and the bootloader ABORTED-rollback path itself (deferred canary). Official StickS3 manual facts recorded in the incident file: side button long press = download mode, single click = power on / reset (a chip-internal reset returns to the latched download mode — esptool's RTS reset does not leave it), double click = power off; HAT2-Bus pin 10 = G43 (UART0 TX), 12 = G44 (UART0 RX), 1 = GND — the only PHY-independent full console. Device state at wrap: `ff1f7d2` on app1 (VALID, confirmed), `254fb7e` on app0; branch `agent/ble-hid-pointer` green, not merged (pending acceptance items above). Reference noted for later review: github.com/Teapot174/AirMouseS3.

## 2026-08-21T16:06:22Z — HEAD ff83936

Closed the IMU blink-click channel on three independent lines of evidence and opened the protocol section 8 sensor evaluation. (1) Worn Session A traces (`capture-20260820T173559`, detector-eye-view analysis with the detector's own 350 ms EMA baseline): during genuine stillness the deviation noise floor sits ON the 1.1 dps impulse threshold (median 1.04 dps, 7 impulse-shaped events inside the still control), while the operator's real firm blinks peak at 30-49 dps — far above the 2.5 dps head-motion veto, which was active 24-31% of the blink stages and cancelled the patterns being performed; V2 additionally produced a head-motion false positive (`head=1`). The gyro channel's operating window (above 1.1, below 2.5 dps) excludes the operator's actual blinks: signal and noise share one modality and band. (2) That `head=1` control-stage sequence is itself a listed section 8 trigger, so the near-eye evaluation opened by the protocol's own rule. (3) Two web research sweeps (six agents; product, literature, and sensor lanes): no shipping product anywhere detects blinks from motion sensors alone — Google Glass wink used a dedicated LTR-506ALS IR-proximity sensor despite carrying a 9-axis IMU, JINS MEME uses EOG electrodes with the IMU only for head motion, every commercial assistive blink switch (including GlassOuse GS12, an IMU head-mouse that still added a separate optical eye sensor) is near-eye IR, and Vision Pro/HoloLens use IR cameras; published IMU-blink work requires sensors adhered to periocular skin. Consensus split: dedicated eye sensor or camera = blink signal, IMU = head pointer plus motion veto — the BMI270 keeps its hardware-validated pointer role and `imu_blink_validated` stays off (fail-closed, no firmware change). Candidate corrections: the VL53L4CD-class ToF named in section 8 is REJECTED on datasheet physics (±7 mm accuracy, ~70 mm practical floor cannot resolve a 2-5 mm lid excursion at ~10 mm); the correct on-device class is integrated IR-proximity ICs (Vishay VCNL4040 / ams-OSRAM TMD2635, Glass's sensor class, IEC 62471 eye-safe at low drive). The active experiment is the free lane first: macOS ships webcam expression-to-click natively (Accessibility > Pointer Control > Alternative Control Methods > "Enable alternative pointer actions", since 10.15.4: expressions including Eye Blink mapped to clicks with Slight/Default/Exaggerated sensitivity) — StickS3 stays the pointer, the Mac camera supplies the click; bench caveats: verify expressions run with Head Pointer off, natural-blink false fires (fallback expressions: Raise Eyebrows / Open Mouth), and lens-glare robustness. Ranked fallbacks: EyeCommander (MIT webcam blink switch), macOS Dwell Control, then a custom MediaPipe FaceLandmarker helper (`eyeBlinkLeft/Right` blendshapes — the model family Android's production Camera Switches ships — fused with EAR, duration- and confidence-gated, clicking via CGEventPost with Accessibility permission). Worn captures remain valuable as pointer and veto-channel fixtures. Same-day hardware-validated result: the operator configured the native path live (macOS Tahoe 26, FaceTime HD camera; the expression trigger is added via the "+" button choosing Facial Expression — the Reassign button only captures keys/switches) with Eye Blink at Exaggerated mapped to Left Click, and real clicks worked — the recognized gesture is a deliberate long firm closure of both eyes ("deeply close both for some time"), which is inherently distinct from natural blinks; operator verdict "works decent once you get the hang". Sensitivity (Slight/Default/Exaggerated) is the per-user calibration dial: the right setting is the lightest that never false-fires. Noted for fixture metadata: the StickS3 rides the LEFT temple, while `round1.json` labels mounts `glasses_right_temple` — correct the mount labels before any promotion. Stage 2 (StickS3 over USB as pointer plus camera click) is the next experiment. Research plus one live validation recorded; only documentation changed.

## 2026-08-20T20:03:23Z — HEAD 7970a26

Dissected the public upstream `tix-life/Colibrino` (GPL-3.0, created 2020-07-15, 51 stars; last code change Jan 2023, README edits through Jan 2025; `colibrino.com.br` 302-redirects to the repo, so the repo is the only prose documentation). Finding: this fork carries the full upstream history and all 25 upstream files byte-identical — legacy AVR firmware, `LimparCalibracao/`, three STL enclosures, both doc images, LICENSE — except `README.md`, which is deliberately extended locally (7,348 → 11,344 bytes); nothing needs importing. Upstream's sole open issue #6 (2023, unanswered) asks for Bluetooth operation, independent demand for the cable-free direction. Source-level learnings extracted (TCRT5000 synchronous IR demodulation, derivative-edge blink detection with hysteresis and per-user calibration, hand-fitted sensor distance, derivative Mahony yaw/pitch cursor mapping with deadzones and wrap correction, EEPROM gyro-bias calibration with buzzer feedback and the LimparCalibracao recovery path, relay/dwell alternate actuation, unshipped lateral-tilt scroll and head-gesture roadmap) and recorded in the new repo `CLAUDE.md` together with the lineage URL. Documentation only; no code changed.

## 2026-08-18T17:01:40Z — HEAD 254fb7e

Hardware-validated the wireless telemetry path and passed the no-cable equivalence acceptance; worn sessions are unblocked. The fourth authorized OTA (`6fff220`) uploaded and booted correctly, but the sanctioned `--tcp` capture tool was refused twice with "connection closed before the telemetry greeting": the accept gate `if (incoming)` invokes `WiFiClient::operator bool()` = `connected()`, whose zero-length recv probe latches false on the capture host's immediate `shutdown(SHUT_WR)` FIN (which arrives back-to-back with the handshake ACK, almost always before the loop task polls accept), so the freshly accepted client was closed un-greeted. An A/B probe proved it on hardware: connecting without the immediate half-close received `EVENT,TELEMETRY,CONNECTED,6fff220,dropped=0` and a later half-close did not end the stream, so only the accept gate was wrong. Fixed with an `fd() >= 0` gate (`254fb7e`), re-reviewed adversarially from three lenses against the installed 2.0.17 core sources (no finding above LOW; the only behavioral delta is a dead-on-arrival RST connection attaching for up to about a second until the first drain `send()` errno detaches it, exactly the design's sole detach rule), and the fifth authorized OTA (`254fb7e`) then verified end-to-end with the real tool. Bench equivalence on `capture-20260818T133312` (three untouched bench probe runs plus one 40 second free-run block, all on the table): per-stage effective rate 168.1-172.3 Hz against the USB baseline's 164.2-168.8 Hz (delta +1.9% to +2.4%, within the 5% bound; wireless lost nothing), `DROPPED` 0 over 30,393 lines, 0 unknown lines, device-time usec-gap p50 about 4.99 ms on both transports with tighter TCP tails (max 34 ms vs 40 ms), `split_sessions` plus replay parity 4/4 MATCH, and the free-run block (11,912 rows at 170.3 Hz) exercised the double-tap entry, tap stop, and `RESULT ... free=` tail on hardware for the first time. Operational observations: the free-run conversion window is the 3 second `PREPARE_STILL` screen and a hesitant human misses it - instruct a quick knock-knock double tap (three misses before the first success); battery drained roughly 0.7%/min under telemetry plus display (47% at session boot, 36% after the bench set), so a worn session should start at 30% or better; one table bump landed during the free-run block (recorded, harmless for equivalence, and the session was never a fixture candidate); the free-run fixture remains promotion-blocked by the `FREE_RUN_SCHEMA_NOTE` until the core branch's labels schema gains `CAPTURE_FREE_RUN`. One of the three USB baseline files sits nested at `.device-backups/logs/logs/device-monitor-260814-234754.log`. `PORT_PLAN.json` T15 is now `implemented_and_hardware_validated`.
